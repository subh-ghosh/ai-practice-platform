    ...
    public StatisticsDto getStatistics(String email) {
        ...
        List<Answer> gradedAnswers = allAnswers.stream()
                .filter(a -> List.of("CORRECT", "INCORRECT", "CLOSE").contains(a.getEvaluationStatus()) &&
                        a.getQuestion() != null && a.getQuestion().getGeneratedAt() != null && a.getSubmittedAt() != null)
                .toList();
        double averageAnswerTimeSeconds = 0.0;
        if (!gradedAnswers.isEmpty()) {
            double totalSeconds = gradedAnswers.stream()
                    .mapToLong(answer ->
                            Duration.between(answer.getQuestion().getGeneratedAt(), answer.getSubmittedAt()).toSeconds()
                    )
                    .filter(seconds -> seconds > 0)
                    .sum();
            long validAnswerCount = gradedAnswers.stream()
                    .filter(answer -> Duration.between(answer.getQuestion().getGeneratedAt(), answer.getSubmittedAt()).toSeconds() > 0)
                    .count();
        ...
        return answersByDay.entrySet().stream()
                ...
                .map(entry -> {
                    ...
                    double avgSpeed = dailyAnswers.stream()
                            .filter(a -> a.getQuestion() != null && a.getQuestion().getGeneratedAt() != null)
                            .mapToLong(a -> Duration.between(a.getQuestion().getGeneratedAt(), a.getSubmittedAt()).toSeconds())
                            .filter(s -> s > 0)
                            .average()
